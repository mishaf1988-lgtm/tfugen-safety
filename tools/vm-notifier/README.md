...
```
השאלות: סוג התיבה ששולחת · הכתובת ששולחת · הסיסמה/מפתח · לאן לשלוח (ברירת מחדל sviva@tapugan.co.il). להחליף הגדרות: להריץ שוב את אותה פקודה.
- **1 = Gmail / Google Workspace** — **סיסמת אפליקציה** (Google Account → Security → App passwords).
- **2 = Microsoft 365 / Outlook** — סיסמת התיבה; ייתכן שמנהל ה-IT יצטרך להפעיל SMTP AUTH.
- **3 = Brevo (API ב-HTTPS)** — **לשרתי ענן שחוסמים SMTP (DigitalOcean, Vultr…)**, מזוהה בשגיאה `timed out`. חשבון חינם ב-https://www.brevo.com → **Senders & IPs → Senders → Add a sender** עם הכתובת ששולחת (Brevo שולחת אליה קישור אישור, לוחצים) → **SMTP & API → API Keys → Generate** → מדביקים במתקין. השולח מופיע כ-sviva@tapugan.co.il.
