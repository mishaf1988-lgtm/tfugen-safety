...
(async () => {
  ...
  const e = { actions: await vis('.topbar-actions'), menu: await vis('#menu-btn'), search: await vis('#gsearch-btn'), notif: await vis('#notif-btn'), tasks: await vis('#my-tasks-btn'), more: await vis('#more-menu-btn'), logo: await page.evaluate(() => { const r = document.querySelector('.topbar-left img').getBoundingClientRect(); return { h: Math.round(r.height), cx: Math.round(r.left + r.width / 2) }; }) };
  check('trustee mode: menu / search / notifications / my-tasks / ⋯ are all gone', e.actions !== 'visible' && e.menu !== 'visible' && e.search !== 'visible' && e.notif !== 'visible' && e.tasks !== 'visible' && e.more !== 'visible', e);
