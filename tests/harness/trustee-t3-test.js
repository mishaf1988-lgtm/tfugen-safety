...
(async () => {
  ...
  check('header ⋯: roster · trustee link · task catalogue · winner · CSV export · hazards to Excel · print · open the trustee screen', hm.length === 8 && /רשימת הנאמנים/.test(hm[0]) && /קישור לנאמנים/.test(hm[1]) && /קטלוג המשימות/.test(hm[2]) && /זוכה החודש/.test(hm[3]) && /CSV/.test(hm[4]) && /מפגעים לאקסל/.test(hm[5]) && /הדפס/.test(hm[6]) && /מסך הנאמן/.test(hm[7]), hm);
