...
(async () => {
  ...
  console.log('\n3b. the default is a lock on every open');
  {
    ...
    // 12 hours, then 30 minutes. A phone that has never chosen anything walks
    // in when used recently, and is asked after half an hour away. «every open» is still
    ...
    check('...because the default is 30 minutes', r.mins === 30, r.mins);
    await p1.close();
    const p1b = await boot({ idleMin: 31, lockMin: null });
    const rb = await state(p1b);
    check('31 minutes away, no choice made: the password is asked for', rb.login === 'flex' && rb.isAdmin === false && rb.signedOut === 1, rb);
    await p1b.close();
  ...
  console.log('\n9. the setting is reachable, and it saves');
  {
    ...
    check('...showing what is in force (30 minutes by default)', r.shown === '30', r);
  ...
  console.log('\n10. the delay is the manager\'s to set, not each user\'s');
  {
    ...
    check('...and a value sent anyway is refused (still ' + r.after + ')', r.after === 30, r);
    await p.close();
  }
