...
(async () => {
  ...
  // 3b lived here: the notice inside a generated report, and where it sat.
  // Since 2026-09-21 that button builds a PDF instead, and the notice only
  // appears when it cannot -- so its placement is covered where that path
  // is exercised, in report-pdf-test.
  ...
  console.log('\n6. the print button inside every generated report');
  {
    ...
    // The injected script grew a PDF builder on 2026-09-21 and now opens with
    // _rptMsg; _tryPrint is still in it, at the end.
    const withFn = (src.match(/<head><meta charset="[^"]+"><meta name="viewport"[^>]*><script>function _rptMsg/gi) || []).length;
