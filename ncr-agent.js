/* NCR AI Agent - TFUGEN SAFETY v2.1 */
window._nad=[];
window._naf={};
window._PC={'קריטי':{bg:'#3d0000',b:'#ff2222',c:'#ff3333'},'גבוהה':{bg:'#3d1a00',b:'#ff6600',c:'#ff7700'},'בינונית':{bg:'#2d2a00',b:'#f0c000',c:'#f0c000'},'נמוכה':{bg:'#001a0d',b:'#00aa44',c:'#00cc55'}};
window._SC={'סגור':'#00cc55','פתוח':'#ff4444','בטיפול':'#f0a000'};
window._gpc=function(p){return window._PC[p]||{bg:'#2d2a00',b:'#f0c000',c:'#f0c000'};};
window._gsc=function(s){return window._SC[s]||'#7a9ab8';};

window.openNCRAgent=function(){
  document.getElementById('ncr-agent-modal').style.display='flex';
  window.ncrAgentLoad();
};

window.ncrAgentLoad=function(){
  ...
  .then(function(res){
    window._nad=Array.isArray(res.d)?res.d:[];
    var ce=document.getElementById('ncr-agent-count');
    if(ce)ce.textContent=res.ct?res.ct.split('/')[1]:window._nad.length;
    window.ncrAgentRender();
  }).catch(function(){L.innerHTML='<div style="text-align:center;padding:40px;color:#ff4444">Error loading data</div>';});
};

window.ncrAgentRender=function(){
  var L=document.getElementById('ncr-agent-list');
  if(!window._nad.length){L.innerHTML='<div style="text-align:center;padding:40px;color:#4a6a8a">No data</div>';return;}
  var h='';
  window._nad.forEach(function(n){
    var pc=window._gpc(n.p),sc=window._gsc(n.s),an=window._naf[n.id];
    ...
    h+='</div><div style="font-size:10px;color:#5a7a9a">'+(n.a||'')+(n.o?' · '+n.o:'')+(n.u?' · '+n.u:'')+'</div></div>';
...
window.ncrAgentSel=function(id){
  var n=window._nad.find(function(x){return x.id===id;});
  if(!n)return;
  var P=document.getElementById('ncr-agent-panel');
  P.style.display='block';
  var pc=window._gpc(n.p),sc=window._gsc(n.s);
  ...
  var an=window._naf[id];
  if(an&&!an.err){h+=window._ncrFmt(an,pc);}
...
window._ncrFmt=function(an,pc){
  var h='';
  if(an.risk){var rc=window._PC[an.risk]||pc;
    h+='<div style="background:'+rc.bg+';border:1px solid '+rc.b+';border-radius:8px;padding:8px 12px;display:flex;justify-content:space-between;align-items:center;margin-bottom:7px"><span style="font-size:11px;color:#aaccee">AI Risk</span><span style="font-size:14px;font-weight:800;color:'+rc.c+'">'+an.risk+'</span></div>';
  }
  [{i:'&#128269;',t:'סיבת שורש',col:'#4a9adf',v:an.root_cause},{i:'&#9889;',t:'פעולה מיידית',col:'#ff6600',v:an.immediate_action},{i:'&#128737;',t:'מונעת',col:'#9a4adf',v:an.preventive}].forEach(function(s){
    if(!s.v)return;
    h+='<div style="background:#111416;border:1px solid '+s.col+'33;border-right:3px solid '+s.col+';border-radius:8px;padding:9px 12px;margin-bottom:7px"><div style="font-size:11px;font-weight:700;color:'+s.col+';margin-bottom:4px">'+s.i+' '+s.t+'</div><div style="font-size:12px;color:#aaccee;line-height:1.6">'+s.v+'</div></div>';
  });
  if(an.corrective_actions&&an.corrective_actions.length){
    h+='<div style="background:#0d1a0d;border:1px solid #1a4a1a;border-right:3px solid #00cc55;border-radius:8px;padding:9px 12px;margin-bottom:7px"><div style="font-size:11px;font-weight:700;color:#00cc55;margin-bottom:7px">&#10003; CAPA</div>';
    an.corrective_actions.forEach(function(a,i){h+='<div style="display:flex;gap:6px;margin-bottom:5px;font-size:12px;color:#aaccaa"><span style="color:#00cc55">'+(i+1)+'.</span><span>'+a+'</span></div>';});
    h+='</div>';
  }
  return h;
};

window.ncrAgentAI=function(id){
  var n=window._nad.find(function(x){return x.id===id;});
  if(!n)return;
  document.getElementById('ncr-agent-panel').innerHTML+='<div style="text-align:center;padding:20px;color:#4a6a8a">&#129302; Analyzing...</div>';
  var pr='You are a safety expert at Tfugen Industries. Analyze this NCR and respond ONLY in Hebrew.\nNCR: '+n.num+'\nDescription: '+(n.d||'')+'\nArea: '+(n.a||'')+'\nPriority: '+(n.p||'')+'\nStatus: '+(n.s||'')+'\n\nJSON only (no backticks):\n{"risk":"קריטי|גבוהה|בינונית|נמוכה","root_cause":"","immediate_action":"","corrective_actions":["","",""],"preventive":""}';
  fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json'},
    ...
    window._naf[id]=JSON.parse((d.content&&d.content[0]?d.content[0].text:'').replace(/```json|```/g,'').trim());
    window.ncrAgentRender();window.ncrAgentSel(id);
  }).catch(function(){window._naf[id]={err:true};window.ncrAgentSel(id);});
};

window.ncrAgentAnalyzeAll=function(){
  var P=document.getElementById('ncr-agent-panel');
  P.style.display='block';
  P.innerHTML='<div style="text-align:center;padding:30px;color:#4a6a8a">&#128269; Analyzing '+window._nad.length+' NCRs...</div>';
  var byA={},byP={},byS={};
  window._nad.forEach(function(n){byA[n.a]=(byA[n.a]||0)+1;byP[n.p]=(byP[n.p]||0)+1;byS[n.s]=(byS[n.s]||0)+1;});
  var sample=window._nad.slice(0,50).map(function(n){return '['+n.num+'] '+(n.d||'')+' | '+(n.a||'')+' | '+(n.p||'');}).join('\n');
  var pr='Analyze '+window._nad.length+' NCRs for Tfugen Industries. By area: '+JSON.stringify(byA)+'. By priority: '+JSON.stringify(byP)+'. By status: '+JSON.stringify(byS)+'.\nSample:\n'+sample+'\n\nRespond in Hebrew with:\n1. **דפוסים חוזרים**\n2. **תחומי סיכון**\n3. **המלצות**\n4. **התראה דחופה**';
  fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json'},
...
console.log('NCR Agent v2.1 loaded');