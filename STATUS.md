...
- [x] **Environmental Aspects (ISO 14001:6.1.2)** — דף חדש `pg-easp` + טבלה `env_aspects` (id, aspect, activity, impact, significance, legal_ref, controls, owner, review_date, notes, ts) + RLS. רישום היבטים סביבתיים שדורשים שליטה. UI: רשימה ממוינת לפי משמעותיות (גבוהה/בינונית/נמוכה) עם badges צבעוניים, מודאל יצירה/עריכה עם 9 שדות, ניווט במודולים → "הנהלה" → "🌳 היבטים סביבתיים". מיגרציה הורצה אוטומטית.
