...
| 6 | 🔴 **פרויקט Vercel חי עדיין מחובר ל-repo** | **אומת 21/09** | CLAUDE.md אומר ש-Vercel נסגר ב-2026-05-03. הוא לא נסגר. הוא מנסה לפרוס **בכל דחיפה**, והיום נגמרה לו המכסה: `Resource is limited - try again in 24 hours (more than 100, code: "api-deployments-free-per-day")` על PR #712.
**אין שום קובץ Vercel ב-repo** (אין `vercel.json`, אין `.vercelignore`, אין `/api`) — הפריסה מופעלת מצד Vercel דרך חיבור ה-GitHub של הפרויקט, ולכן **שום שינוי ב-repo לא יעצור אותה**.
**הסיכון האמיתי אינו הרעש:** אם קיימת כתובת `*.vercel.app` פעילה, היא מגישה את הבילד האחרון שהצליח — עותק **קפוא** של אפליקציית בטיחות. עובד שסימן אותה כמועדפת ימשיך לדווח לאותו Supabase מגרסה שלא מקבלת תיקונים.
⛔ **תיקון, 21/09: מיכאל משתמש בעותק הזה.** «אני משתמש באפליקציה השנייה פחות ב-vercel». **אל תציע למחוק אותו.** ההמלצה הקודמת כאן נכתבה מתוך הנחה שהוא פרויקט יתום, והיא הייתה שגויה.
**מה שכן נכון לגביו:** Vercel אינו מריץ Pages Functions של Cloudflare, ואין ב-repo תיקיית `/api`. לכן בעותק ההוא **11 נקודות הקצה מחזירות 404**: `claude` (כל ה-AI), `trustee-notify` (כפתור «שלח בדיקה»), `wa-send`/`wa-status`/`wa-templates`, `create-user`/`delete-user`/`rename-user`/`reset-password`/`self-recovery`, `_securityselftest`.
**מה כן עובד שם:** כל מה שמדבר ישירות ל-Supabase — רשומות, דוחות, דיווחי נאמנים. **גם ההתראות המיידיות עובדות משם**, כי הן נורות מטריגר בבסיס הנתונים ולא מהדפדפן.
**הסיכון שנשאר:** קוד ישן. הפריסה שם נכשלת מאז שנגמרה המכסה, כלומר העותק קפוא מאחור ולא מקבל תיקונים. |
