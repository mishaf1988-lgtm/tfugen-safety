...
- [ ] **🔒 הרץ migration ב-Supabase**: `migrations/2026-04-25_move_is_admin_manager_to_private.sql` — מעביר את `is_admin_manager()` מסכמה `public` ל-`private` (PostgREST לא חושף `private` ב-REST). מחליף את הפונקציה בכל 26 הפוליסות `*_admin_manager_all`. נכרך ב-`BEGIN/COMMIT` — אם משהו נכשל, כלום לא משתנה. **אחרי הרצה**: לוודא ש-(1) Security Advisor מציג 0 errors + 35 warnings (ללא 2 ה-`*_security_definer_function_executable`), (2) admin עדיין רואה 375 NCRs + דפי בדיקות ציוד + משימות + audit_log, (3) מצב עובד שטח עדיין מאפשר דיווח near_miss.
