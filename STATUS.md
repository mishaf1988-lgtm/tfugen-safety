...
| 7 | 🟠 **הוואטסאפ נשלח בתבנית של «תקרית בטיחות»** | ממתין לאישור Meta | התבנית המאושרת היחידה היום היא `tfugen_incident_alert`, והטקסט הקבוע שלה («🚨 תקרית בטיחות», «נא לטפל מיידית») **אינו ניתן לשינוי מהקוד**. מיכאל ביקש ניסוח רגוע: «מפגע או ליקוי ולא תקרית». **לחיצה אחת:** מודולים → 🔔 הגדרות התראות → **📤 הגש תבנית**. מגיש את `tfugen_safety_report` ל-Meta מהשרת. הקוד כבר מנסה אותה ראשונה ויעבור אליה ברגע שתאושר, **בלי שינוי קוד ובלי פריסה**. פרטים והגשה ידנית: `project-files/WHATSAPP-TEMPLATE.md` |
