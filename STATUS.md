...
| 6 | 🟠 **פרויקט Vercel ישן עדיין מחובר ל-repo** | **אומת 21/09** | CLAUDE.md אומר מאז מאי ש-Vercel נסגר. הוא לא נסגר: הוא מנסה לפרוס בכל דחיפה, והיום נגמרה לו המכסה — `Resource is limited ... api-deployments-free-per-day` על PR #712.<br>**מיכאל, 21/09: «בזה אני משתמש Cloudflare, בשני לא נגעתי הרבה זמן».** כלומר Cloudflare הוא הייצור, והעותק של Vercel אינו בשימוש.<br>**אין שום קובץ Vercel ב-repo** (אין `vercel.json`, אין `/api`) — הפריסה מופעלת מצד Vercel דרך חיבור ה-GitHub, ולכן **שום שינוי ב-repo לא יעצור אותה**. רק הדשבורד.<br>**מה יש בעותק ההוא בפועל:** Vercel אינו מריץ Pages Functions, ולכן 11 נקודות קצה מחזירות שם 404 (AI, trustee-notify, שלוש נקודות WhatsApp, חמש של ניהול משתמשים, ו-self-test). כל מה שמדבר ישירות ל-Supabase כן עובד, וגם ההתראות המיידיות — הן נורות מטריגר ב-DB ולא מהדפדפן. לכן הפער בלתי נראה מבפנים.<br>**המלצה:** למחוק את הפרויקט. קודם Settings → **Domains** לוודא שאין שם דומיין אמיתי; אם רק `*.vercel.app`, אז Settings → **Delete Project**. מחיקה מסירה גם את הכתובת — ניתוק Git בלבד משאיר עותק **קפוא** של אפליקציית בטיחות באוויר, שמדבר לאותו Supabase ולא מקבל תיקונים. **פעולה בלתי הפיכה בתשתית של מיכאל: ההחלטה שלו, לא של Claude.** |
