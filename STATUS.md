...
**Last updated**: 2026-05-04 (Performance Phase 0+1+2+3+4+5(3/5)+6 shipped · 26 PRs · web-vitals · debounce · memoize · Cache-Control · SSE keepalive · Storage cacheControl · RLS perf wrap (initPlan) · poll 2→10min · per-page render-skip · rAF debounce realtime · hourly SW update + soft pill · SW auto-version via CF Pages function · WhatsApp restored end-to-end after Meta App Switch-to-Live · privacy.html + user-data-deletion.html · **Legal Register periodic review** + dashboard widget · **Incident 5-Why** · plan in project-files/PERFORMANCE-PLAN-2026-05-04.md)
...
- [x] **Incident 5-Why (PR #366, 2026-05-04)** — ISO 45001 §10.2. עמודה חדשה `inc.five_why text` (migration `2026-05-04_inc_five_why.sql` — דורש הרצה ידנית). UI: callout צהוב inline ב-`showView` של תקריות (כי `m-inc` הוא create-only — אין edit modal). כפתור "+ הוסף ניתוח" / "✏️ ערוך" → `prompt()` עם תבנית 1..5 → שמירה דרך `sbUpd`. עד הרצת ה-migration: עובד מקומית בלבד, ה-INSERT לענן יושמט בשקט.
