...
- [ ] 📱 **דחיפת התזכורת בוואטסאפ לנאמנים עצמם** — אותו חישוב (`_truBehind`), שליחה דרך `/api/trustee-notify`. **חסום בשניים:** (א) עמודת `ph` ב-`trustees` ריקה לכל 8 — צריך שדה טלפון בעורך הרשימה + שמיכאל יקליד; (ב) המספר הנוכחי הוא מספר הבדיקה של מטא ושולח רק ל-5 נמענים — צריך לחבר את המספר המשרדי של תפוגן לחשבון ה-WhatsApp הקיים (אימות בשיחה קולית; התבניות המאושרות עוברות איתו כי הן שייכות לחשבון ולא למספר). מיכאל: להוסיף כשיהיה במשרד.
