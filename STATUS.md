...
**מה לעשות (מיכאל בלבד, בדשבורד):** vercel.com → הפרויקט `tfugen-safety` → Settings → **Domains** לבדוק קודם שאין שם דומיין אמיתי — ואם יש רק `*.vercel.app`, **Delete Project**. מחיקה מסירה גם את הכתובת; ניתוק Git בלבד משאיר את העותק הקפוא באוויר |
