...
- [x] **Tasks render fix (PR #402)** — הוסר `content-visibility:auto;contain-intrinsic-size:0 48px` מ-`.tbl-wrap tbody tr` (הוסף ב-PR #336 לאופטימיזציה לטבלאות ארוכות). באג: בטבלאות קצרות (1-3 שורות) הדפדפן מקצה placeholder 48px אבל לא מצייר את התוכן. דף משימות הציג כותרת ריק. תוקן.
