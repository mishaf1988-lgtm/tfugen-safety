...
- [x] **נאמני בטיחות — שלב 2: הכרזת זוכה + היסטוריה** (#579) — טבלה `trustee_winners` (זוכה אחד לחודש, id דטרמיניסטי `win_YYYY-MM` — הכרזה חוזרת מעדכנת את אותה שורה ולא יוצרת שנייה) + RLS כמו `trustees` (policy אחד לכל פעולה). **מנהל:** פס «הכרז זוכה» מעל לוח הניקוד + מודאל עם בורר (המוביל הזכאי נבחר מראש), הערה והיסטוריית זוכים; ⋯ → «זוכה החודש + היסטוריה». **נאמן:** באנר זהוב עם הזוכה שהוכרז, מעל «מי מוביל החודש». **שובר השוויון «מי שטרם זכה»** נכנס לדירוג — זכיות נספרות **רק מחודשים קודמים**, כך שהכרזה לעולם לא משנה את דירוג אותו חודש עצמו — וצ׳יפ «טרם זכה» מופיע בלוח. הניקוד נשמר מוקפא בשעת ההכרזה (דיווח מאוחר לא משכתב היסטוריה). T3 23/23 (5 בדיקות חדשות) + **כל 24 הסוויטות ירוקות**. ⚠ המיגרציה טרם הורצה — ראה «פעולה ידנית נדרשת».
...
- [ ] 🏆 **הרץ migration ב-Supabase**: `migrations/2026-09-19_trustee_winners.sql` (SQL Editor → Paste → Run). ללא זה «הכרז זוכה» ישמור מקומית אבל לא יסתנכרן, והנאמנים לא יראו את הזוכה. אימות: `SELECT policyname,cmd FROM pg_policies WHERE tablename='trustee_winners';` — 4 שורות.
