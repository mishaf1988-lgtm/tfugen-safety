...
//   POST /api/vitre?op=close                    staff: { appointmentId, description } -> closes the Vitre task that the notification (appointment) generated
...
// POST (or PUT) against Vitre with a JSON body. Same return shape as vitreGet, never throws.
function vitrePost(env, path, body) { return vitreSend(env, 'POST', path, body); }
function vitrePut(env, path, body) { return vitreSend(env, 'PUT', path, body); }
async function vitreSend(env, method, path, body) {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), UPSTREAM_TIMEOUT_MS);
  try {
    const resp = await fetch(VITRE_BASE + path, {
...
// ---- Closing the Vitre task when the finding closes here (26/09) ----
// /task/get has no filter (Swagger read 26/09: PageNumber, PageSize only) and
// pages oldest-first, so the task is found in two steps: get-review-result of
// the appointment carries actionId (= the task, seen 23/09 on the refresher
// form), and /task/get/{id} confirms that task was generated by THIS
// appointment. Only then is it ours to close. Fallback: scan the pages.
const TASK_SCAN_PAGE = 200;
const TASK_SCAN_MAX_PAGES = 40;
function taskRow(d) { return d && d.task && typeof d.task === 'object' ? d.task : d; }
async function findTaskForAppointment(env, apptId) {
  const calls = [];
  const rr = await vitreGet(env, '/appointmetResult/get-review-result/' + apptId);
  const actionId = rr.ok && rr.json && Number.isInteger(rr.json.actionId) ? rr.json.actionId : null;
  calls.push({ step: 'get-review-result', status: rr.status, actionId });
  if (actionId) {
    const d = await vitreGet(env, '/task/get/' + actionId);
    const t = taskRow(d.json);
    calls.push({ step: 'task/get/' + actionId, status: d.status, generatedBy: t && t.generatedByAppointmentId });
    if (d.ok && t && t.generatedByAppointmentId === apptId) return { task: t, calls };
  }
  for (let page = 1; page <= TASK_SCAN_MAX_PAGES; page++) {
    const r = await vitreGet(env, '/task/get?PageNumber=' + page + '&PageSize=' + TASK_SCAN_PAGE);
    if (!r.ok || !Array.isArray(r.json)) { calls.push({ step: 'scan', page, status: r.status, error: r.networkError || r.text || null }); break; }
    const hit = r.json.find(t => t && t.generatedByAppointmentId === apptId);
    if (hit) { calls.push({ step: 'scan', page, found: hit.id }); return { task: hit, calls }; }
    if (r.json.length < TASK_SCAN_PAGE) { calls.push({ step: 'scan', page, end: true }); break; }
  }
  return { task: null, calls };
}
...
export async function onRequest({ request, env }) {
  ...
  // ---- Close the Vitre task when the finding closes here (26/09) ----
  // Body: { appointmentId (from the "Vitre SMS #N" note on the finding),
  // description }. Same gate as notify: the channel that opens tasks is the
  // one that closes them. The task is closed only when Vitre confirms it was
  // generated by that very appointment; an already-closed task is left alone.
  if (op === 'close') {
    ...
    if (!Number.isInteger(apptId) || apptId <= 0 || String(body.appointmentId).trim() !== String(apptId)) return jsonResp({ error: 'appointmentId (positive integer) required' }, 400, cors);
    const description = String((body && body.description) || '').trim().slice(0, 1000) || 'Closed in Tapugan Safety';
    const found = await findTaskForAppointment(env, apptId);
    if (!found.task) return jsonResp({ error: 'no Vitre task generated by appointment ' + apptId, appointmentId: apptId, lookup: found.calls }, 404, cors);
    const t = found.task;
    if (t.closeDate) return jsonResp({ ok: true, appointmentId: apptId, taskId: t.id, alreadyClosed: true, closeDate: t.closeDate }, 200, cors);
    const r = await vitrePut(env, '/task/close', { taskId: t.id, isHandled: true, taskHandled: { description } });
    if (!r.ok) return jsonResp({ error: 'vitre ' + (r.status || 'unreachable'), detail: String((r.json && (r.json.message || r.json.description)) || r.text || r.networkError || '').slice(0, 300), upstreamStatus: r.status, taskId: t.id }, r.status >= 400 ? 502 : 504, cors);
    const success = !!(r.json && r.json.success);
    if (!success) return jsonResp({ error: 'vitre did not confirm the close', taskId: t.id, result: r.json }, 502, cors);
    return jsonResp({ ok: true, appointmentId: apptId, taskId: t.id, title: t.title || null, alreadyClosed: false }, 200, cors);
  }
