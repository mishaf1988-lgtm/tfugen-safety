...
**עדכון 23/09 (אחרי האבחון)**: הטופס מוגש כסגירת המשימה (`closedByAppointmentId`), הקובץ מגיע כ-SAS URL ל-5 דקות ולכן מועתק ל-Storage שלנו בשרת (`incidents-photos/vitre-tr-<taskId>`). זו כתיבה ל-Storage שלנו, לא ל-Vitre. מבנה `get-review-result` עדיין לא נצפה; חילוץ המחלקות הוא best-effort ומתוקן אחרי הריצה הראשונה.
