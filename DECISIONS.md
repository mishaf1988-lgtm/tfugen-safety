...
## 2026-09-22 — «מכשיר מנהל» נקבע לפי סשן משוחזר, לא לפי כניסה עם סיסמה

**החלטה**: `proceed()` כותב את `tfgn_mgr_device` ברגע שסופאבייס מחזיר סשן של משתמש עם אימייל — **לפני** בדיקת הנעילה, ולא רק ב-`_finishLogin`. סשן משוחזר שנפל בנעילה עדיין מוכיח של מי הטלפון.
**סיבה**: מיכאל נחת על מסך הקוד של הנאמנים שלוש פעמים, דרך שני תיקונים (#758, #759). שני התיקונים היו נכונים ולא יעילים: הנעילה ברירת-המחדל מאז 21/09 היא «בכל פתיחה», אז `admin` יורד ל-false בכל פתיחה, ומשם הקוד נפל לענף הקיוסק כי הסימון נכתב רק בכניסה עם סיסמה שהוא לא עשה מאז. תיקון שמותנה בפעולה שהמשתמש לא יודע שהוא צריך לעשות אינו תיקון.
**למה הבדיקות לא תפסו**: `session-restore-test.js` מריץ את כל המקרים תחת `lockMin: 720` כדי שהשחזור בכלל ייבחן. זה מסתיר בדיוק את ברירת-המחדל שהמשתמש חי בה. נוסף `lockEveryOpen` וסעיף Z3 שמריץ את המקרה של מיכאל כפי שהוא. **לקח**: כשהמשתמש אומר «אין שינוי» אחרי תיקון שעבר בבדיקות, לבדוק קודם באילו הגדרות הבדיקה רצה, לא באילו הגדרות התיקון אמור לעבוד.
**אלטרנטיבות שנדחו**: לבטל את הדביקות של `EMP_KEY` (הנאמנים תלויים בה כשפותחים את ה-PWA המותקן) · לבקש ממיכאל להיכנס פעם אחת עם סיסמה (עובד, אבל דורש ממנו לדעת משהו שלא כתוב על המסך).
**קישורים**: #758 · #759 · #760

## 2026-09-21 — Vercel נסגר באמת הפעם

**החלטה**: פרויקט ה-Vercel `tfugen-safety` נמחק. Cloudflare Pages הוא הייצור היחיד.
**סיבה**: CLAUDE.md הכריז מ-2026-05-03 ש-Vercel נסגר, וזה היה נכון לגבי ה-repo בלבד — תיקיית `/api` אכן נמחקה. **הפרויקט עצמו נשאר מחובר ל-GitHub ופרס בכל דחיפה עוד ארבעה חודשים.** התגלה ב-21/09 כשנגמרה לו המכסה היומית והוא שלח מייל כישלון על PR #712.
**למה זה היה חשוב ולא רק רעש**: `tfugen-safety.vercel.app` הגיש עותק ציבורי שני של אפליקציית בטיחות, מול אותו Supabase. Vercel אינו מריץ Cloudflare Pages Functions, ולכן שם 11 נקודות קצה החזירו 404 — כל ה-AI, כפתור בדיקת ההתראות, שלוש נקודות WhatsApp, חמש של ניהול משתמשים, וה-self-test. כל השאר עבד, **כולל ההתראות המיידיות** (הן נורות מטריגר ב-DB ולא מהדפדפן), ולכן הפער היה בלתי נראה מבפנים.
**למה מחיקה ולא ניתוק Git**: ניתוק עוצר את הבילדים ומשאיר את הכתובת עונה עם הבילד האחרון — גרסה קפואה שלא מקבלת תיקונים, שאדם עלול לסמן כמועדפת.
**אלטרנטיבות שנדחו**: לתקן מה-repo. לא אפשרי — אין בו שום קובץ Vercel (`vercel.json`, `/api`), והפריסה מופעלת מצד Vercel דרך חיבור ה-GitHub.
**מי ביצע**: מיכאל, בדשבורד, אחרי שאימת שהכתובת היחידה היא `*.vercel.app` ולא דומיין של תפוגן. פעולה בלתי הפיכה בתשתית שלו, ולכן שלו לבצע.
**לקח**: הצהרה ב-CLAUDE.md על תשתית שנסגרה מתארת את ה-repo, לא את העולם. השורה שם עודכנה כדי לומר את שניהם.

## 2026-09-21 — התראת מפגע: שני מקורות, נמען אחד

**החלטה**: `near_miss` מקבל טריגר משלו שקורא לאותה פונקציה `/api/trustee-notify` עם `{"src":"near_miss"}`. מפת `SOURCES` מחזיקה לכל טבלה את ה-select, את מבחן «האם בכלל שולחים», את נירמול שמות העמודות ואת הניסוח. **שני המקורות שולחים לאותו נמען** - זה ששמור תחת `trustee_hazard` ב-`notification_prefs`.
**סיבה**: מיכאל, 21/09: «אני צריך ככה שהם ממלאים מפגע בסיור הוא עולה מייד וגם כמעט ונפגע, שלא יצא מצב שכמה אנשים מעלים ומגלים מאוחר». `near_miss` היא דווקא הטבלה שכל אחד יכול לכתוב אליה, לא רק נאמן, ועד היום דיווח כזה לא הגיע לאיש עד שהממונה פתח את האפליקציה.
**למה נמען אחד ולא שניים**: מיכאל צריך למלא מספר טלפון וכתובת מייל **פעם אחת**. שדה שני הוא שדה שני שיישאר ריק.
**למה `src` הוא מפתח במפה ולא טקסט**: שם הטבלה מגיע ל-query string. הוא נבחר מתוך מפה קבועה ולעולם לא נלקח כמחרוזת מהבקשה.
...
## 2026-09-21 — דחיפת קובץ מהשרת ל-OneDrive: נדחה, והסיבה טכנית

**החלטה**: **לא** בונים שרת שדוחף קובץ ל-OneDrive של מיכאל. אם צריך «אונליין לאקסל», הכיוון הוא שאקסל **מושך** מנקודת קצה, לא שהשרת דוחף.
**סיבה**: החיבור הקיים ל-OneDrive הוא `msal.PublicClientApplication` - SPA public client. טוקני הרענון שלו ב-Entra ID חיים **24 שעות ומקושרים לדפדפן**. שרת לא יכול להחזיק אותם. זה לא עניין של מאמץ, זה לא אפשרי עם הרישום הקיים.
**מה כן היה נדרש**: רישום אפליקציה נפרד ב-Azure עם client secret, ניהול סוד, והסכמה חד-פעמית. מיכאל אינו מפתח.
**מה שכן נבנה**: `_hazExportXlsx` - כפתור שמייצר גיליון מפגעים אחד (ליקויי נאמנים + כמעט ונפגע). PR #701.
**נשאר פתוח**: אם בכל זאת רוצים קובץ שמתעדכן לבד, שתי דרכים - נקודת קצה `/api/hazards` שאקסל מושך ממנה (דורשת **החלטת אבטחה**: אקסל לא עושה OAuth, ולכן ההגנה היא טוקן ארוך בכתובת), או להוסיף `מפגעים.xlsx` להעלאה היומית הקיימת ל-OneDrive (מתעדכן רק כשהאפליקציה נפתחת). **לא לבנות את הראשונה בלי אישור מפורש.**

## 2026-09-21 — טקסט שנשלח לאנשים: רק תווים שיש במקלדת

**החלטה**: בכל טקסט שאדם קורא - מדריכים, PDF, מיילים, הודעות WhatsApp, הודעות באפליקציה - אין מקף ארוך, מקף עברי, «», גרשיים מסולסלים, חצים, נקודה אמצעית, או «...» כתו אחד. מקף רגיל, גרשיים רגילים, נקודתיים או פסיק במקומם. **אימוג'י מותר** כשהוא מופיע על כפתור באפליקציה.
**סיבה**: בקשה מפורשת של מיכאל, 21/09. התווים האלה אינם במקלדת שהוא מקליד בה.
**למה אימוג'י כן**: המדריך אומר «לחצו על ✅ תקין». בלי האימוג'י הוא מפנה לכפתור אחר מזה שעל המסך.
**איך זה נאכף**: שתי בדיקות, ושתיהן על ה**פלט** ולא על הקוד. `build-pdf.js` (`KEYBOARD_ONLY`) מסיר את ה-HTML וה-CSS ואז בודק את הטקסט; `trustee-notify-test.mjs` בודק את כותרת המייל, גוף המייל, החלק הטקסטואלי ופרמטרי ה-WhatsApp שנשלחים בפועל, עם נתוני בדיקה נקיים מהתווים האלה. הסיבה שזו בדיקה ולא כלל בראש: התווים חוזרים בכל ניסוח מחדש, ואי אפשר לראות מקף ארוך בקריאה.
**לא חל על**: `STATUS.md`, `DECISIONS.md`, הודעות commit ו-PR.
**קישורים**: PR #700, PR #704, `CLAUDE.md` סעיף «שפה ותקשורת».

## 2026-09-21 — המייל מוגבל לכתובת אחת: זו הגדרה של Resend, לא באג

**החלטה**: המייל יוצא רק אל `mishaf1988@gmail.com` עד שאחד משני הדברים קורה. **לא מנסים לעקוף בקוד.**
**סיבה**: חשבון Resend בלי דומיין מאומת שולח רק לכתובת שאיתה נפתח החשבון. אנחנו שולחים מ-`onboarding@resend.dev` (ברירת המחדל כש-`RESEND_FROM` אינו מוגדר). התשובה המלאה: «You can only send testing emails to your own email address (mishaf1988@gmail.com)».
**שתי הדרכים קדימה**: (א) לשנות את כתובת החשבון ב-Resend לכתובת היעד הרצויה; (ב) לאמת דומיין ב-resend.com/domains ולהגדיר `RESEND_FROM` ב-Cloudflare Pages - זה גם פותח שליחה לכמה נמענים.
**מה כן תוקן בקוד**: ה-403 הזה החזיר קיר JSON באנגלית חתוך ב-160 תווים, ומיכאל נאלץ לקרוא את `net._http_response` ב-SQL Editor כדי להבין. עכשיו הוא מחזיר משפט בעברית שנוקב בכתובת שכן תתקבל ואומר מה לעשות. PR #703.
**הערה תפעולית**: הגדרות ההעברה של Gmail **אינן נגישות מאייפון** - לא באפליקציה, לא באתר לנייד, וגם `?nocheckbrowser` לא עוקף. נדרש מחשב. אל תשלח את מיכאל לנסות שוב.

## 2026-09-20 — גבול ה-Storage לסשן האנונימי: לפי תחילית שם הקובץ

**החלטה**: `incidents-photos` מוגבל כך שסשן אנונימי (הקיוסק של הנאמנים) רואה וכותב **רק** שמות שמתחילים ב-`tru-ph-`; חשבון עם אימייל ולא-אנונימי מקבל את כל הדלי כמקודם. DELETE — לחשבון אמיתי בלבד. `migrations/2026-09-20_storage_trustee_scope.sql`, **טרם הורצה**.
**סיבה**: ארבע ה-policies מאפריל נתנו לכל `authenticated` את כל הדלי, ו-sign-in אנונימי ב-Supabase הוא בדיוק `authenticated` — אותה מלכודת של #615, בצד שלא נבדק. מיכאל קבע את האילוץ: «נאמנים צריכים להעלות תמונות בסיור». קריאה בקוד הראתה שהקיוסק צריך שלושה דברים בלבד — העלאה, העלאת תמונת «אחרי», וצפייה בתמונות שלו (`_sign` מייצר קישור חתום ב-POST ל-`/object/sign/`, ו-RLS מתייחס לזה כ-SELECT). DELETE הוא לא אחד מהם: האפליקציה לא מוחקת מ-Storage בשום מקום.
**למה תחילית ולא בעלות (`owner`)**: סשן אנונימי מקבל uid חדש לכל מכשיר. נאמן שסוגר ממצא ממכשיר אחר לא היה רואה את התמונה שלו. תחילית עובדת בלי תלות בזהות.
**הסיכון שנלקח**: הגבול הוא תכונה של **הקוד**, לא של ה-DB — `_attachBtn` מעביר את מזהה האזור כ-prefix, מזהי האזור באים מ-`_truPhId`, ו-`_fileUpload` בונה מהם את שם הקובץ. אם אחד משלושתם ישתנה, ה-policy תשתוק והתמונות ייעלמו. לכן `tests/harness/storage-scope-test.js` מקבע את שלושתם, ואומת בשבירה מכוונת.
**נכשל סגור**: טוקן בלי claim של `is_anonymous` נחשב אנונימי, ונדרש גם אימייל — אותה בדיקה של `requireUser()` ב-`functions/_shared.js`.

## 2026-09-20 — תקרת רינדור: אחרי המיון, המונים לא נחתכים, והייצוא מרים אותה

**החלטה**: עמודי המשימות והתפוגות מציירים `_RENDER_CAP=200` שורות. התקרה חלה **אחרי** המיון שכל עמוד עושה; המונים מעל הרשימה מחושבים מכל הסט; ו-`_exportCSV` מרים את התקרה, מייצא ומחזיר.
**סיבה**: נמדד ב-375px — משימות 16,548 אלמנטים, תפוגות 9,647, כל השאר 1,500–4,300. אחרי: 3,662 ו-2,451.
**העיקרון**: תקרה ששינתה את המספרים בשקט גרועה מעמוד איטי. לכן המונים לא נחתכו מלכתחילה — **ואת הייצוא פספסתי ב-#617**, כך שה-CSV מעמוד התפוגות הכיל 200 מתוך 800 ליום אחד. תוקן ב-#625, ושורת הכותרת התחתונה קיבלה `class="cap-note"` כדי ששום ייצוא לא יקרא אותה כרשומה.
**אלטרנטיבות שנדחו**: pagination אמיתית (הרבה יותר קוד, ומנהל בטיחות לא רוצה לדפדף); להוריד את התקרה אחרי הרגרסיה (הבעיה היתה הייצוא, לא התקרה).

## 2026-09-20 — `notifications_log`: חלון של 500 בסנכרון, יצוא מלא מהשרת, בלי כפתור מחיקה

**החלטה**: `_sbLimit={notifications_log:500}` — הטבלה היחידה בכל האפליקציה שנחתכת בסנכרון. ה-CSV מושך מהשרת את היומן **המלא** ברגע הלחיצה. כפתור «נקה יומן מקומי» **הוסר**.
**סיבה**: הטבלה append-only וגדלה בלי סוף; נמשכה **במלואה** בכל סנכרון כדי לצייר 20 שורות. הכפתור עשה `DB.notifications_log=[]` + `sdb()` — אבל הטבלה ב-`_SDB_SKIP` וכלום לא נמחק מהשרת, אז הסנכרון הבא החזיר הכל תוך פחות מדקה (נבדק חי: 2→0→2). מיכאל: «להוריד את הכפתור».
**למה אף טבלה עסקית לא נחתכה**: להסתיר NCR ישן או ציוד שיצא משימוש כי הרשימה התארכה זה אובדן נתונים שקט. הבדיקה מוודאת את זה במפורש.
**למה היומן נשאר על השרת**: הוא הראיה שההתראות יצאו. מחיקה שלו היא שאלת רשומות, לא שאלת ממשק.

## 2026-09-20 — דוח המוכנות לביקורת: ירוק רק עם רשומות **וגם** פעילות ב-12 חודשים

**החלטה**: `isoAuditReport()` מסמן סעיף כמכוסה רק כשיש רשומות **וגם** רשומה מ-12 החודשים האחרונים. שורה בלי תאריך לעולם לא נחשבת עדכנית. כל ⚠️/❌ נושא את המספר שמאחוריו.
**סיבה**: מבקר לא מקבל מרשם שהפסיקו לנהל. דוח שמסמן ✅ על יומן תרגילים שהרשומה האחרונה בו בת 400 יום נותן ביטחון-שווא בדיוק במקום שבו הוא הכי מזיק.
**הסתייגות שכתובה על הדף עצמו**: הוא אינו תחליף לביקורת — הוא מראה איפה הראיה קיימת ואיפה צריך להשלים.
...
**בוטל 2026-09-20 (באותו יום)**: מיכאל — «לא רלוונטי, אפשר להוריד». `.claude/skills/typesafe-ai/` נמחק ב-#614. הרשומה נשארת כאן כי יומן החלטות לא משכתבים — השיקול שלא להריץ התקנה מ-marketplace על סמך טקסט מודבק נשאר נכון ושווה לזכור. **לא שונה שום קוד של האפליקציה** — לא כשהוא נוסף ולא כשהוא הוסר.
...
**סיבה**: מיכאל, 2026-09-19: «לא צריך כרגע שליחה, בטל את זה כרגע». במקביל התברר שהוא ממילא לא עבד: כל הריצות שנדגמו מ-2026-05-22 ואילך נכשלו, הלוג מ-29/07 מראה `RESEND_KEY:` ריק → `401 API key is invalid`, ו-GitHub השבית את התהליך (`disabled_inactivity`) ב-29/07. הסרת ה-`schedule` היא הדרך לוודא שהוא לא מתעורר לבד אם בעתיד יתווסף מפתח למטרה אחרת.
**מה זה לא אומר**: התראות הליקויים מנאמני בטיחות (Supabase trigger → `functions/api/trustee-notify.js` → Meta/Resend) **לא נגעו ועובדות**. הן משתמשות ב-`RESEND_KEY` שב-**Cloudflare**, שהוא משתנה נפרד לגמרי מה-secret של GitHub. לא לבלבל ביניהם.
**אלטרנטיבות שנדחו**: למחוק את ה-workflow והסקריפט (מיכאל אמר «כרגע» — הוא עשוי לרצות את זה בחזרה); להשאיר את התזמון ולסמוך על זה ש-GitHub השבית (ההשבתה זמנית — פעילות בריפו מחזירה אותה); לתקן ולהפעיל (מיכאל ביקש את ההפך).
**להחזיר**: צריך **שתי פעולות** — לבטל הערה משתי שורות `schedule`, **וגם** להוסיף secret בשם `RESEND_KEY` ב-Settings → Secrets and variables → Actions. אחת בלי השנייה מחזירה בדיוק את הכישלון השקט שנמשך 4 חודשים.
**קישורים**: #602 · `.github/workflows/daily-reminders.yml` · run 30436213831 (הלוג שהוכיח את הסיבה).

---

## 2026-09-19 — ה-service worker מגיש את המעטפת מהמטמון, לא מהרשת

**החלטה**: `/`, `/index.html`, `/manifest.webmanifest`, `/icon.svg`, `/logo.jpg` וכל ניווט — **cache-first עם רענון ברקע** (`waitUntil`). כל השאר נשאר כשהיה: קבצים שאינם מעטפת עדיין network-first עם נפילה למטמון באופליין, ו-Supabase / `/api/*` / Claude / Google Fonts עוברים ישר לדפדפן.
**סיבה**: נמדד קודם. הקובץ הוא 1.35MB גולמי אבל **248KB בלבד על הקו** (brotli), והפענוח+ההרצה של כל ה-JS הוא ~110ms — שניהם בסדר, והספריות הכבדות כבר נטענות לפי דרישה. הבעיה הייתה ש-network-first הוריד את 248KB **בכל פתיחה**, גם כשלא השתנה כלום. מדידת A/B בדפדפן אמיתי: בקליטה חלשה (1.5Mbps) **1512ms → 96ms**, ב-4G טוב 317ms → 96ms — והזמן החדש לא תלוי במהירות הרשת בכלל. זה קריטי במפעל, שם הקליטה חלשה בדיוק במקומות שבהם מדווחים.
**למה זה לא מחזיר את באג הגרסה התקועה**: העדכון עובר דרך `/sw.js`, לא דרך `index.html`. `/sw.js` הוא `no-cache` והבייטים שלו נושאים את ה-commit sha, לכן בכל דיפלוי ה-worker החדש מותקן, מושך את המעטפת החדשה ב-install, מוחק את ה-cache הישן ושולח `sw-updated` — הפיל «גרסה חדשה זמינה». בפועל הפיל נעשה בעל משמעות עכשיו: קודם הוא הופיע אחרי שהמשתמש כבר קיבל את הבייטים החדשים מהרשת. המחיר: בפתיחה הראשונה אחרי דיפלוי רואים את הבילד הקודם לכמה שניות עד שהפיל קופץ.
**אלטרנטיבות שנדחו**: לפצל את `index.html` ל-JS/CSS נפרדים עם hash (סותר את «קובץ יחיד, בלי build step» — כלל 5+6 ב-CLAUDE.md); להקטין את הקובץ ע"י הסרת `\uXXXX` (סותר כלל 1, ו-brotli דוחס את ההירוב בין כה וכה); cache-first גם לשאר הקבצים (בלי רווח, עם סיכון); להפוך את גיליון הפונטים ללא-חוסם (שינוי ויזואלי — לשאול את מיכאל קודם).
**קישורים**: #593 · `functions/sw.js.js` · `tests/harness/sw-cache-test.mjs` (32 בדיקות על ה-worker האמיתי, כולל מחזור דיפלוי מלא).

---

## 2026-09-19 — שורות דיבאג: לא למחוק את מה שמעקב אחרי עבודות רקע

**החלטה**: `console.log` שהוא רעש אתחול — למחוק. `console.log` שמתעד עבודת רקע שרצה בשקט (גיבוי, dispatcher מייל, OneDrive) — להעביר ל-`dlog()` שכבוי כברירת מחדל (`?debug=1` או `tfgnDebug(true)`). `console.warn`/`console.error` — לא לגעת.
**סיבה**: המשימה נרשמה כ«למחוק 23 שורות דיבאג», אבל קריאה של כולן הראתה שרק 3 הן רעש טהור. ה-22 האחרות הן הנראות היחידה לתהליכים שאין להם מסך — אם הגיבוי היומי מדלג בשקט, בלעדיהן אין דרך לדעת למה. מחיקה הייתה נקיון למראה ואובדן בפועל.
**אלטרנטיבות שנדחו**: למחוק את כולן (אובדן נראות); להשאיר את כולן (קובר את ה-`console.warn` שמסמנים תקלות אמיתיות, ושתיים מהן הדפיסו payload של דיווח עם שם וטלפון); דגל build-time (אין build step בפרויקט).
**קישורים**: #594 · `tests/harness/console-quiet-test.js` (14 בדיקות) · אותה גישה של `?perf=1` שכבר הייתה בקוד.

---

## 2026-09-19 — תמונות אופליין: IndexedDB, וסימון `pending:` במקום חסימת הדיווח

**החלטה**: כישלון **רשת** בהעלאת קובץ שומר אותו ב-IndexedDB ומחזיר סימון `pending:<id>` כערך התמונה, במקום `null`. `_phDrain` רץ **בראש** `_obDrain`, כך שהתמונות עולות לפני השורות. שגיאת שרת אמיתית (403, קובץ גדול) נשארת שגיאה ולא נכנסת לתור.
**סיבה**: תמונה חובה לליקוי, אז כישלון העלאה חסם את **כל** הדיווח — בשטח, בלי קליטה, שם האפליקציה הכי נחוצה. זה גם מה שוואטסאפ ידע לעשות והאפליקציה לא. הסדר (תמונות לפני שורות) חשוב כי טריגר ה-webhook שולח וואטסאפ על INSERT: שורה שעלתה עם סימון הייתה מייצרת התראה עם קישור שבור.
**אלטרנטיבות שנדחו**: localStorage (תמונה דחוסה 100–300KB — מכסת 5MB משותפת עם התור והמטמון, הייתה נשברת אחרי 2–3); base64 בתוך שורת ה-outbox (מנפח כל פעולה ושובר את ה-PATCH); לוותר על חובת התמונה כשאין רשת (הכלל מהמסמך — «דיווח בלי תמונה לא נספר»); להכניס גם שגיאות שרת לתור (403 לא יתוקן בהמתנה, והמשתמש היה חושב שנשלח).
**קישורים**: #588 · `offline-photo-test.js` (15 בדיקות, כולל שורה שכבר נשלחה ו-403 שלא נכנס לתור) · המלצה 1 מ«רעיונות גדולים» בסקירת ה-UX.

---

## 2026-09-19 — זוכה החודש: זכיות נספרות רק מחודשים קודמים

**החלטה**: `_truWinsBefore(u,m)` סופר זכיות **רק בחודשים קטנים מ-`m`**, והוא שובר השוויון השלישי ב-`_truBoard` (אחרי ניקוד ואחרי מפגעים שנסגרו). הזוכה נשמר עם `pts` מוקפא ו-id דטרמיניסטי `win_YYYY-MM`.
**סיבה**: אלמלא כן, הכרזת הזוכה לחודש `m` הייתה **משנה את הדירוג של אותו חודש ברגע שמרעננים** — הזוכה הטרי היה צונח בלוח שלו עצמו, וזה נראה כמו באג. הקפאת `pts` נובעת מאותה סיבה: דיווח מאוחר או סגירת מפגע לא אמורים לשכתב היסטוריה שהוכרזה.
**אלטרנטיבות שנדחו**: ספירת כל הזכיות כולל החודש הנוכחי (משנה דירוג למפרע); עמודת `wins` מצטברת על `trustees` (נשברת במחיקת הכרזה ודורשת עדכון שתי טבלאות); הכרזה אוטומטית ב-1 לחודש (מיכאל רוצה לאשר ידנית — לפעמים יש שיקול דעת).
**קישורים**: #579 · `migrations/2026-09-19_trustee_winners.sql` · הארנס `trustee-t3-test.js` (5 בדיקות חדשות) · SPEC שלב 2.

---

## 2026-09-19 — תזכורת נאמנים: במסך הבית, לא בדחיפה לנאמנים

**החלטה**: התזכורת על נאמנים שטרם השלימו 5 משימות מוצגת למיכאל במסך הבית (#589). **לא** נשלחת דחיפה בוואטסאפ לנאמנים עצמם.
**סיבה**: הדחיפה דורשת שני דברים שמיכאל לא יכול לספק — מספרי הטלפון של 8 הנאמנים (עמודת `ph` ריקה), ובעיקר מספר עסקי של תפוגן בחשבון מטא, כי המספר הנוכחי הוא מספר בדיקה שמוגבל ל-5 נמענים. הקו המשרדי היה פותר את זה טכנית, אבל יש עלות הודעות חודשית והארגון לא מממן. מיכאל החליט לוותר (2026-09-19).
**מה עובד במקום**: מיכאל רואה במסך הבית מי מפגר ובכמה, ופונה אליהם בקבוצת הוואטסאפ הקיימת מהמספר האישי שלו. אותה תוצאה, אפס עלות, בלי תלות במטא.
**אם זה יחזור**: החישוב כבר בנוי (`_truBehind()`), אז ההוספה היא שליחה דרך `/api/trustee-notify` בלבד. **אבל אל תציע את זה מיוזמתך** — זו החלטה של מיכאל, לא מגבלה טכנית.
**קישורים**: #589 · `_truBehind` ב-index.html · תבניות מטא מאושרות ושייכות לחשבון (יעברו עם כל מספר עתידי).

## 2026-09-19 — outbox: סיום העלאה ממזג, לא דורס (תיקון מרוץ)

**החלטה**: `_obDrain` מסיים במיזוג: מוריד מה שנשלח, שומר את מה שנכשל (עם `tries`), ושומר **כל פעולה שנכנסה לתור בזמן ההעלאה**; אם נכנסו כאלה — מפעיל סבב נוסף.
**סיבה**: עד עכשיו סיום ההעלאה כתב את ה-snapshot שנלקח בתחילתה (`_obSet(remaining)`), כך שכל `sbIns`/`sbUpd` שקרה בזמן round-trip אחד (למשל שמירת סיור נאמן = 3 שורות, או "שמור NCR + צור משימה") **אבד לצמיתות** — השורה נשארה ב-DB המקומי ונעלמה ב-pull הבא (מקור אמת = שרת, #541). נמצא ע"י ההארנס של טופס הסיור.
**אלטרנטיבות שנדחו**: תור-בתור (serialize `_obDrain` עם promise chain) — פותר אבל משנה תזמון בכל האפליקציה; נעילת `_obPush` בזמן drain — חוסם שמירות.
**קישורים**: #565 · הארנס `outbox-race-test.js` (מריץ את המקור האמיתי של `_obDrain`).
